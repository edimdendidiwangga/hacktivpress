...
<div id="front" class="ui items segment padtop-6 container">
  ...
  <div class="ui divided items">
	...
	<select class="ui dropdown" v-model="searchDataCategory">
		<option v-for="post in listPostCat" v-bind:value="post.category">{{ post.category }}</option>
	</select>  <button class="ui primary button" v-on:click="getPostCategory">Search By Category</button>
  <br>
  <select class="ui dropdown" v-model="searchDataAuthor">
		<option v-for="post in listPostAut" v-bind:value="post.author.name">{{ post.author.name }}</option>
	</select> <button class="ui primary button" v-on:click="getPostAuthor">Search By Author </button>
  <br>
  ...
  methods: {
    ...
    showAddArticle() {
      ...
      $('#modalAddArticle').modal('show')
		...
		getPostCategory() {
			this.$router.push('/category/' + this.searchDataCategory);
    },
    getPostAuthor() {
			this.$router.push('/author/' + this.searchDataAuthor);
    },
		listCategory() {
			let self = this;
      axios.get('http://localhost:3000/articles')
